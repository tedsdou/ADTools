...
($countryoption -eq '13'))
    ...
    switch ($countryoption) {
        1 {
            $CyprusEmployeesOUs = ($AllOUs |
                Where-Object { (($_.DistinguishedName -like '*Employees*') -or
...
($_.DistinguishedName -like '*Cyprus*') }).DistinguishedName
            ...
            $CyprusComputersOUs = ($AllOUs |
                Where-Object { ($_.DistinguishedName -like '*Computers*') -and
($_.DistinguishedName -like '*Cyprus*') }).DistinguishedName

            $CyprusGroupsOUs = ($AllOUs |
                Where-Object { (($_.DistinguishedName -like '*DistributionGroups*') -or
($_.DistinguishedName -like '*SecurityGroups*')) -and
($_.DistinguishedName -like '*Cyprus*') }).DistinguishedName

            $CyprusServiceOUs = ($AllOUs |
                Where-Object { ($_.DistinguishedName -like '*ServiceAccounts*') -and
($_.DistinguishedName -like '*Cyprus*') }).DistinguishedName

            $CyprusServersOUs = ($ServersOUs |
                Where-Object { (($_.DistinguishedName -like '*2008*') -or
...
($_.DistinguishedName -like '*Cyprus*') }).DistinguishedName
            ...
            $CyprusOtherOUs = ($OtherOUs |
                Where-Object { (($_.DistinguishedName -like '*Clusters*') -or
...
($_.DistinguishedName -like '*India*') }).DistinguishedName
    ...
    switch ($objectoption) {
        1 {
            foreach ($currentuserou in $activeuserou) {
        ...
        2 {
            foreach ($currentdesktopou in $activedesktopou) {
        ...
        3 {
            foreach ($currentgroupou in $activegroupou) {
        ...
        4 {
            foreach ($currentserviceou in $activeserviceou) {
        ...
        5 {
            foreach ($currentserverou in $activeserverou) {
    ...
    do {
        $answer = Read-Host -Prompt 'Do you want to run delegation script again for another object or country? (y or n)'
